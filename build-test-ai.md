...
- **向量库连接器**：支持 Qdrant、Milvus 等向量数据库的连接配置
...
CREATE TABLE vector_db_configs (
    ...
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,
    name VARCHAR(100) NOT NULL,
    db_type VARCHAR(50) NOT NULL, -- 'qdrant', 'milvus', 'chroma'
