...
- **THEN** the document status transitions to `completed`
- **AND** chunk statistics are persisted
- **AND** each persisted chunk includes source metadata sufficient for replay when available
...
- **AND** when the chunk was produced from a multimodal block, the `source` metadata includes replay fields (e.g., `block_type`, `page`, `bbox_norm`, `page_image_path`, `crop_image_path`)
